...
```
serv_project/
├── build.sh              # Compile firmware (and optionally simulate via FuseSoC)
...
│   ├── sim_main.cpp      # Verilator C++ testbench (monitors PC transitions)
│   ├── trace_dump.py     # Convert trace.bin → human-readable trace
│   └── compare_traces.py # Merge sim log + trace → final report
│
├── log/                  # All generated outputs
│   ├── sim_log.txt       # Per-instruction cycle cost (from sim_main.cpp)
│   ├── sim_wave.vcd      # VCD waveform (open with GTKWave)
│   ├── trace_dump.txt    # Symbol-resolved PC trace
│   ├── compare_result.txt# Merged trace with cycle costs
│   └── instr_count.log   # Static instruction count breakdown
│
├── firmware.elf          # Linked ELF binary (generated by build.sh)
├── firmware.bin          # Raw binary (generated by build.sh)
├── firmware.hex          # Verilog hex image (loaded by simulation RAM)
│
├── fusesoc_libraries/    # SERV RTL + FuseSoC core definitions
│   └── serv/
│       ├── rtl/          # SERV CPU core Verilog sources
│       ├── servant/      # SERV SoC wrapper (RAM, GPIO, timer, mux)
│       ├── bench/        # Testbench & UART decoder
│       └── sw/           # Linker script (link.ld) & makehex.py
│
└── build/                # FuseSoC build artifacts (trace.bin etc.)
```
