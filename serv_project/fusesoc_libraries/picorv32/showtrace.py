...
import os, shutil
objdump_bin = "/home/chenyoo/riscv_benchmark/tools/riscv64/usr/bin/riscv64-unknown-elf-objdump"
if not os.path.exists(objdump_bin):
    objdump_bin = shutil.which("riscv64-unknown-elf-objdump") or shutil.which("riscv32-unknown-elf-objdump") or "riscv32-unknown-elf-objdump"
with subprocess.Popen([objdump_bin, "-d", elf_filename], stdout=subprocess.PIPE) as proc:
    while True:
