...
// o_mode is additionally gated on i_cnt_en so that after stage two ends (o_init
// flips back to 1 until the next instruction is fetched) the accumulator is held
// idle instead of re-accumulating garbage that would corrupt the RF writeback.
always @(*) begin
    if (i_is_customized)
        o_mode = i_init & i_cnt_en;
    else
        o_mode = 1'b0;
end
