...
module serv_rf_ram_if
  ...
  (
   ...
   //o_wr_busy: asserted for ~34 cycles after a write request so the rest of the
   //core knows a serial writeback is occupying the RF interface.
   reg [5:0] 		  wr_busy_cnt;
   assign o_wr_busy = (wr_busy_cnt != 6'd0);
   ...
   always @(posedge i_clk) begin
	...
	rgate <= i_rreq;
	 ...
	 rcnt <= {{CMSB-1{1'b0}},i_wreq,1'b0};
      ...
      if (i_wreq)
	wr_busy_cnt <= 6'd1;
      else if (wr_busy_cnt != 6'd0) begin
	 wr_busy_cnt <= wr_busy_cnt + 6'd1;
	 if (wr_busy_cnt == 6'd6)
	   wr_busy_cnt <= 6'd0;
      end
